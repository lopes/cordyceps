...
At a high level, the process begins with a primary layer of symmetric encryption. We generate a random AES-GCM 256-bit key and a nonce, which are then used to encrypt the plaintext file content. The [aes_gcm](https://crates.io/crates/aes_gcm) module automatically appends an authentication tag to the resulting ciphertext due to the [Galois/Counter Mode (GCM)](https://en.wikipedia.org/wiki/Galois/Counter_Mode). It achieves this using a block cipher in Counter (CTR) mode, which eliminates the need for padding and makes the output size predictable. This is a crucial step for integrity and authenticity, ensuring the data hasn't been tampered with.
...
1. A [Curve25519](https://en.wikipedia.org/wiki/Curve25519) main 256-bit public key is provided to our function, which is a static public key belonging to the intended recipient.
